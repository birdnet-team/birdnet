...
- Removed a fixed ~1 s barrier from every `run_arrays(..)` call and from worker shutdown. Workers wait for work in `sem_filled.acquire(timeout=1.0)`, which a `multiprocessing.Event` cannot interrupt, so `all_producers_finished` was only observed once the poll interval elapsed. The last producer now releases a permit when it finishes and each worker passes one on as it exits, so they wake immediately and take the `claimed_slot is None` exit that was already there. On a warm session a 3 s clip went from 1069 ms to 39 ms, 9 s from 1134 ms to 95 ms and 30 s from 1358 ms to 315 ms. Workers also observe teardown promptly now instead of surviving the grace period and being SIGTERM'ed one by one (#51).
