...
- A run that produces no predictions at all no longer hangs when a `progress_callback` is used. The performance tracker skipped publishing stats entirely while no worker had reported a timing, so a run whose inputs are all unprocessable produced none — and the progress dispatcher, which only stops once it sees stats flagged as finished, looped forever without setting its finish signal, leaving the call waiting on it indefinitely. The final stats are now always published, so such a run completes and the callback receives a closing update with `finished=True`, no worker stats and zero processed segments (#75).
