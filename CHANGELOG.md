...
- Added support for the BirdNET-Geomodel V3.0 (release v3.0.4, 14,082 classes covering birds, insects, amphibians and mammals) with TFLite/LiteRT (`tf`, INT8/FP16/FP32), ProtoBuf (`pb`), PyTorch/TorchScript (`pt`) and ONNX (`onnx`, FP16/FP32) backends. Load via `birdnet.load("geo", "3.0", <backend>)`. The `pt` and `onnx` backends require the `birdnet[pt]`/`birdnet[onnx]` extras and also let the geo model run without TensorFlow (e.g. on Python 3.14). The `pt` backend uses the TorchScript export added in geomodel v3.0.4 (the `.pt` shipped alongside it is still a training checkpoint) and applies the sigmoid that export omits, so all four backends return the same probabilities (#41).
...
- Added (partial) support for Python 3.14. TensorFlow does not yet ship Python 3.14 wheels, so on 3.14 `birdnet` installs without TensorFlow and supports the models that have a TensorFlow-free backend: the acoustic 3.0 model (`onnx`/`pt`) and the geo 3.0 model (`onnx`/`pt`). The version cap `<3.14` was lifted and `tensorflow` is now only a dependency on Python ≤3.13. TensorFlow-only paths (`tf`/`pb` backends, the acoustic 2.4, geo 2.4 and Perch models) raise a clear, actionable error on 3.14 instead of an `ImportError`. Full support will follow once TensorFlow provides 3.14 wheels (#55).

### Changed

- The V3.0 models now use the versioned taxonomy `taxonomy_v0.2-Jun2026.csv` that the geomodel repository ships since its v3.0.4 release (the unversioned `taxonomy.csv` the previous v3.0.2 pin pointed at no longer exists). It is the taxonomy the geo model was built against - all 14,082 of its label codes resolve in it - and it is also the closer fit for the acoustic V3.0 model, resolving 188 more of its species and agreeing with its own label file in 1,487 English names where the previous taxonomy disagreed. Cached taxonomy and label files are detected as stale and refreshed automatically. Estonian (`et`) was dropped from the V3.0 language list (acoustic and geo): the new taxonomy has no Estonian column, so those names would silently have been the English ones. Where the taxonomy reuses one species code for two species (it does for `y01249` and `grsbop1`), the geo label generation now picks the row whose scientific name matches the label instead of whichever came last.
...
- A pipeline process that dies mid-run is now reported instead of hanging the run forever. The parent waits for one sentinel per worker on the results queue and for each child's finish signal, both without a timeout, because a healthy run legitimately takes as long as the audio requires. A child killed from the outside (typically the OOM killer when memory runs out) or crashing in native code sends neither, so `predict(..)`/`encode(..)` blocked indefinitely with no output at all — indistinguishable from a run still in progress. The parent now checks child liveness once per second while waiting and fails the run with the process name and its exit code, pointing at `n_workers`/`batch_size` as the memory knobs. This covers a child that dies between batches. Two cases are not covered and are tracked separately: a child killed *while processing a batch* holds the ring-buffer lock, which a dying process never releases on Linux and macOS, so the surviving workers block instead (#73); and a child killed mid-write can leave a partial message that makes the teardown drain block, so such a run is now reported and can still hang while closing the session (#77).
- Fixed the progress callback's closing update, which could hang the run or report nothing. The performance tracker decided whether its queues were drained *before* waiting for processing to finish, so everything the workers reported during that wait was dropped — on a short run that is all of it, and the closing update said "finished, nothing processed, 0%" for a run that processed everything. When there was nothing at all to report (e.g. every input unprocessable) it published no closing update, and the progress dispatcher, which only stops once it sees one, looped forever without signalling — leaving the call waiting on it indefinitely. The tracker now drains once more before publishing, so the last update is truthful for every run, and always publishes it. The dispatcher additionally stops when the session is closing, so it can never block `__exit__`, and its finish signal is reset between runs again now that it reliably signals — without which runs after the first received no closing update at all (#75).
...
- V3.0 label files now record the taxonomy they were generated from and are regenerated when it changes. The taxonomy is shared between the acoustic and geo V3.0 models and cached under a generic name, so whichever model downloaded a new one first made it "available" for all the others; their label files were then indistinguishable from current ones and kept serving the localized and English names built from the previous taxonomy. This was only reachable on a taxonomy bump, of which this release is the first (#41).
