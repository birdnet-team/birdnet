...
- V3.0 species names could be silently wrong: two installed versions sharing one app data directory each replaced the other's copy of the shared taxonomy, and the generated label files were only checked against the taxonomy URL a version *expected*, not the file it actually read. Every generated label directory now records the checksums of the inputs it was built from and of the files it produced, so a swapped input is detected whatever caused it. The taxonomy is cached per release instead of under one shared name, and the label files are regenerated once on upgrade — offline, from artifacts already on disk.
