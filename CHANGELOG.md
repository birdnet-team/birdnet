...
- V3.0 species names could be silently wrong when two installed versions shared one app data directory: the generated label files recorded the taxonomy a version *expected*, not the one it actually read. They now record checksums of their inputs and outputs, the taxonomy is cached per release, and existing files are regenerated once on upgrade.
