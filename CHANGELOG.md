...
- A worker killed while processing a batch — the realistic out-of-memory case — no longer wedges the surviving workers on Linux and macOS. The pipeline shares a lock while claiming ring-buffer slots, and a `multiprocessing.Lock` is a POSIX semaphore: a process killed while holding one never releases it, so every other process waiting on it waited for the rest of the run. (Windows recovers on its own there, because a mutex owned by a dead process is abandoned to the next waiter.) Waiters now check every second whether the run has been cancelled and shut down cleanly if it has, so the death is reported by the existing liveness check instead of ending in a hang or in the parent having to terminate the survivors. The same applies to the counter behind the progress display's slot and busy-worker gauges, which is abandoned rather than waited on — the gauges may read a few counts off after a child is killed (#73).
- Teardown after a cancelled run no longer hangs on a message a killed child never finished writing. The parent drains the child-to-parent queues so blocked children can exit, but `Queue.get_nowait()` is not actually non-blocking: once it sees any bytes it waits for the rest of the message, which never arrives, and raises nothing that could be caught. The drain now runs on background threads, one per queue, so it can never stop teardown, and the wait for the log writer — which reads the shared logging queue the same way — is bounded too. A queue whose drain is stuck is deliberately left open, because closing it would free its file descriptor for reuse underneath a blocked read (#77).
