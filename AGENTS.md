...
- CI keys its ~3 GB model cache on the download set — every quoted `https://` URL and `dl_size`/`*_DL_SIZE` literal under `src/birdnet` — so adding or updating a model busts the key while backend edits do not. Declare downloads as quoted literals, or the `Compute model cache key` step in `ci.yml` stops tracking them and pins the key to a stale cache; it fails the job if it finds implausibly few.
