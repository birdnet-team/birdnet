...
class SpeciesTensor:
    ...
    def _alloc(self, F: int, max_chunk_size: int):
        ...
        self._species_probs = np.zeros(
            (F, max_chunk_size, self._top_k), dtype=np.float16)
        self._prediction_mask = np.ones(
            (F, max_chunk_size, self._top_k), dtype=bool)
    ...
    def ensure_capacity(self, needed_n_chunks: int):
        ...
        self._species_ids.resize(
            (self._species_ids.shape[0], needed_n_chunks,
             self._species_ids.shape[2]),
            refcheck=False,
        )
        self._species_probs.resize(
            (self._species_probs.shape[0], needed_n_chunks,
             self._species_probs.shape[2]),
            refcheck=False,
        )
        self._prediction_mask.resize(
            (self._prediction_mask.shape[0], needed_n_chunks,
             self._prediction_mask.shape[2]),
            refcheck=False,
        )
