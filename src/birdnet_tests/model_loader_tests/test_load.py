...
def test_types_with_precisions_are_correct():
  assert (
    load("acoustic", "2.4", "pb", precision=cast(MODEL_PRECISIONS, f"fp{32}"))
    is AcousticPBModelV2_4
  )
  assert (
    load("acoustic", "2.4", "tf", precision=cast(MODEL_PRECISIONS, f"fp{32}"))
    is AcousticTFModelV2_4
  )
  assert (
    load("geo", "2.4", "pb", precision=cast(Literal["fp32"], f"fp{32}"))
    is GeoPBModelV2_4
  )
  assert (
    load("geo", "2.4", "tf", precision=cast(Literal["fp32"], f"fp{32}"))
    is GeoTFModelV2_4
  )
