...
def test_tf_fp16_is_close() -> None:
  model = load("geo", "3.0", "tf", precision="fp16", library="tflite")
  for case_nr, result in predict_test_cases(model):
    ref_case_file = TEST_CASES_REF_DIR / f"{case_nr}.npz"
    ref_result = GeoPredictionResult.load(ref_case_file)
    assert_prediction_results_are_close(
      result, ref_result, case_nr, rtol=0.00001, atol=1e-5
    )


def test_tf_int8_is_close() -> None:
  model = load("geo", "3.0", "tf", precision="int8", library="tflite")
  for case_nr, result in predict_test_cases(model):
    ref_case_file = TEST_CASES_REF_DIR / f"{case_nr}.npz"
    ref_result = GeoPredictionResult.load(ref_case_file)
    assert_prediction_results_are_close(
      result, ref_result, case_nr, rtol=0.001, atol=1e-4
    )
