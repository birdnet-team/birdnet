  ...
  def __init__(
    ...
    self.workers = [
      mp.Process(
        ...
        args=(producer.queue, self._queue, producer.reading_finished, batch_size),
        daemon=True,
      )
...
class ChildWorker:
  def __init__(self, model_path: str, k: int, thresh: float, valid: np.ndarray):
    ...
    self.thresh = thresh
    self.valid = valid
    self.interp = tflite.Interpreter(model_path=str(model_path), num_threads=1)
  ...
  def _infer_topk(self, batch: np.ndarray):
    ...
    print(
      f"Worker {os.getpid()} processed {len(batch)} chunks in {time.perf_counter() - now:.2f}s"
    )
    idx = np.argpartition(pred, -self.k, axis=1)[:, -self.k :]
    sorted_order = np.take_along_axis(pred, idx, axis=1).argsort(axis=1)[:, ::-1]
  ...
  def __call__(self, job_q, out_q, stop, batch_size):
    ...
    while True:
      ...
      if chunks_available:
        try:
          file_idx, chunk_index, audio_chunk = job_q.get()
        except queue.Empty:
          print("Queue empty, no more chunks available.")
          break
        if file_idx is None and chunk_index is None and audio_chunk is None:
          print("Received stop signal, no more chunks available.")
      ...
      undone_work_exists = batch_is_full

      if no_more_chunks_available and unprocessed_chunks_exist:
        undone_work_exists = True

      if undone_work_exists:
        batch = np.array(buffer_audio[:batch_size])

        species_indicies, species_probs, pred_msk = self._infer_topk(batch)

        file_indices = np.array([f[0] for f in buffer_meta])
        chunk_indices = np.array([f[1] for f in buffer_meta])
        res = (file_indices, chunk_indices, species_indicies, species_probs, pred_msk)
        out_q.put(res)
