...
def loadModel(class_output=True):
  ...
  if cfg.MODEL_PATH.endswith(".tflite"):
    ...
    INTERPRETER = tflite.Interpreter(model_path=os.path.join(
      SCRIPT_DIR, cfg.MODEL_PATH), num_threads=cfg.TFLITE_THREADS)
...
def loadMetaModel():
  ...
  # Load TFLite model and allocate tensors.
  M_INTERPRETER = tflite.Interpreter(model_path=os.path.join(
    SCRIPT_DIR, cfg.MDATA_MODEL_PATH), num_threads=cfg.TFLITE_THREADS)
...
def trainLinearClassifier(
  ...
  if not cfg.MULTI_LABEL:
    x_train, y_train, x_val, y_val = birdnet.utils.random_split(x_train, y_train, val_split)
  else:
    x_train, y_train, x_val, y_val = birdnet.utils.random_multilabel_split(
  ...
  if upsampling_ratio > 0:
    x_train, y_train = birdnet.utils.upsampling(x_train, y_train, upsampling_ratio, upsampling_mode)
  ...
  if train_with_mixup and not cfg.BINARY_CLASSIFICATION:
    x_train, y_train = birdnet.utils.mixup(x_train, y_train)

  # Apply label smoothing
  if train_with_label_smoothing and not cfg.BINARY_CLASSIFICATION:
    y_train = birdnet.utils.label_smoothing(y_train)
...
def saveLinearClassifier(classifier, model_path: str, labels: list[str], mode="replace"):
  ...
  if mode == "append":
    labels = [*birdnet.utils.readLines(cfg.LABELS_FILE), *labels]
  ...
  birdnet.utils.save_model_params(model_path.replace(".tflite", "_Params.csv"))
...
def save_raven_model(classifier, model_path, labels: list[str], mode="replace"):
  ...
  with open(model_config, "w") as modelconfigfile:
    ...
    birdnet.utils.save_model_params(model_params)
