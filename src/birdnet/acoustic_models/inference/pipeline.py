...
class AcousticPipelineBase:
  def __init__(
    ...
    self._model_backend: MODEL_BACKENDS = model_backend
    self._model_version: ACOUSTIC_MODEL_VERSIONS = model_version
    self._model_segment_size_s = model_segment_size_s
    self._model_segment_size_samples = int(model_segment_size_s * model_sample_rate)
    self._model_sample_rate = model_sample_rate
    self._model_sig_fmin = model_sig_fmin
    self._model_sig_fmax = model_sig_fmax
    self._model_precision: MODEL_PRECISIONS = model_precision
    self._model_is_custom = model_is_custom

  @abstractmethod
  def get_result_tensor(self) -> TensorBase: ...

  def get_worker(self, device: str) -> WorkerBase: ...

  def get_prediction_result(
    self, result_tensor: TensorBase
  ...
  def prepare_processing(
    ...
    if not 0 <= overlap_duration_s < self._model_segment_size_s:
      raise ValueError(
        f"Value for 'overlap_duration_s' is invalid! It needs to be in interval [0.0, {self._model_segment_size_s})."
      )
    ...
    if self._model_backend == MODEL_BACKEND_TF:
      for d in devices:
        if "GPU" in d:
          raise ValueError(
    ...
    if max_audio_duration_min is not None:
      reserve_n_segments = get_max_n_segments(
        max_audio_duration_min * 60, self._model_segment_size_s, overlap_duration_s
      )
    ...
    rf_audio_samples = RingField(
      ...
      shape=(n_slots, batch_size, self._model_segment_size_samples),
    )
  ...
  def _predict(
    ...
    with (
      ...
      file_analyzer_proc = threading.Thread(
        target=FilesAnalyzer(
          ...
          segment_duration_s=self._model_segment_size_s,
      ...
      producer_processes = [
        mp.Process(
          target=ChildProducer(
            ...
            fmin=self._model_sig_fmin,
            fmax=self._model_sig_fmax,
            max_segment_idx_ptr=max_segment_idx_ptr,
      ...
      elif self._model_backend == MODEL_BACKEND_PB:
      ...
      worker_processes = [
        mp.Process(
          target=self.get_worker(device=devices[i]),
          name=f"Worker-{i}",
          daemon=True,
        )
      ...
      if track_performance:
        ...
        perf_tracker = mp.Process(
          target=PerformanceTracker(
            ...
            segment_size_s=self._model_segment_size_s,
            logging_queue=logging_queue,
            logging_level=logging_level,
    ...
    res = self.get_prediction_result(result)
    del result
    ...
    if show_stats in ("minimal", "progress"):
      bmm = MinimalBenchmarkEmbMeta(
        ...
        model_segment_duration_seconds=self._model_segment_size_s,
      ...
      bmm = FullBenchmarkEmbMeta(
        ...
        model_is_custom=self._model_is_custom,
        model_path=str(self._model_path.absolute()),
        model_species=len(self._model_species_list),
        ...
        model_sig_fmin=self._model_sig_fmin,
        model_sig_fmax=self._model_sig_fmax,
...
class EmbeddingsAcousticPipeline(AcousticPipelineBase):
  def __init__(
    self,
    model_species_list: OrderedSet[str],
    model_path: Path,
    ...
    super().__init__(
      model_species_list=model_species_list,
      model_path=model_path,
      model_backend=model_backend,
      model_version=model_version,
      model_segment_size_s=model_segment_size_s,
...
class ScoresAcousticPipeline(AcousticPipelineBase):
  def __init__(
    self,
    model_species_list: OrderedSet[str],
    model_path: Path,
