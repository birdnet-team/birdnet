...
from birdnet.backends import InferenceBackend, InferenceBackendLoader
...
def predict_species_from_recordings(
  ...
  if top_k is not None and top_k > len(species_list):
    raise ValueError(
      f"top_k cannot be larger than the number of species ({len(species_list)})."
    )

  if top_k is None:
    top_k = len(species_list)
  ...
  if custom_species_list is not None and len(custom_species_list) > 0:
    ...
    for i, species_name in enumerate(custom_species_list):
      assert species_name in species_list
      species_id = species_list.index(species_name)
  ...
  if show_stats in ("minimal", "progress"):
    ...
    bmm = FullBenchmarkMeta(
      ...
      model_is_custom=use_custom_model,
