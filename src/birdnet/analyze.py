"""Module to analyze birdnet.audio samples.
"""
...
import birdnet.audio
import birdnet.config as cfg
import birdnet.model
import birdnet.species
...
def saveResultFile(r: dict[str, list], path: str, afile_path: str):
  ...
  Args:
      ...
      afile_path: The path to birdnet.audio file.
  """
  ...
  if cfg.RESULT_TYPE == "table":
    ...
    high_freq = birdnet.audio.get_sample_rate(afile_path) / 2
...
def combineResults(folder: str, output_file: str):
  ...
  # Read all files
  files = birdnet.utils.collect_all_files(folder, ["txt"], pattern="BirdNET.selection.table")
    ...
    for rfile in files:
      with open(rfile, "r", encoding="utf-8") as rf:
        ...
        try:
          ...
          f_duration = birdnet.audio.getAudioFileLength(f_name, cfg.SAMPLE_RATE)

          birdnet.audiofiles.append(f_name)
...
def getRawAudioFromFile(fpath: str, offset, duration):
  """Reads an birdnet.audio file.

  Reads the file and splits the signal into chunks.

  Args:
      fpath: Path to the birdnet.audio file.

  Returns:
      The signal split into a list of chunks.
  """
  # Open file
  sig, rate = birdnet.audio.openAudioFile(fpath, cfg.SAMPLE_RATE, offset,
                                          duration, cfg.BANDPASS_FMIN, cfg.BANDPASS_FMAX)

  # Split into raw birdnet.audio chunks
  chunks = birdnet.audio.splitSignal(sig, rate, cfg.SIG_LENGTH, cfg.SIG_OVERLAP, cfg.SIG_MINLEN)
...
def predict(samples):
  ...
  prediction = birdnet.model.predict(data)

  # Logits or sigmoid activations?
  if cfg.APPLY_SIGMOID:
    prediction = birdnet.model.flat_sigmoid(
...
def analyzeFile(item):
  ...
  fileLengthSeconds = birdnet.audio.getAudioFileLength(fpath, cfg.SAMPLE_RATE)
  results = {}
  ...
  try:
    ...
    # Write error log
    print(f"Error: Cannot analyze birdnet.audio file {fpath}.\n", flush=True)
    birdnet.utils.writeErrorLog(ex)
...
if __name__ == "__main__":
  ...
  # Parse arguments
  parser = argparse.ArgumentParser(description="Analyze birdnet.audio files with BirdNET")
  parser.add_argument(
  ...
  cfg.CODES = loadCodes()
  cfg.LABELS = birdnet.utils.readLines(cfg.LABELS_FILE)
  ...
  if args.classifier is not None:
    ...
    if args.classifier.endswith(".tflite"):
      ...
      cfg.LABELS = [line.split(",")[1] for line in birdnet.utils.readLines(cfg.LABELS_FILE)]
  ...
  if cfg.LATITUDE == -1 and cfg.LONGITUDE == -1:
    ...
    cfg.SPECIES_LIST = birdnet.utils.readLines(cfg.SPECIES_LIST_FILE)
  else:
    cfg.SPECIES_LIST_FILE = None
    cfg.SPECIES_LIST = birdnet.species.getSpeciesList(
  ...
  if os.path.isdir(cfg.INPUT_PATH):
    cfg.FILE_LIST = birdnet.utils.collect_audio_files(cfg.INPUT_PATH)
